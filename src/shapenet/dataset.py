...
class ShapeNet(InMemoryDataset):
    ...
    def download(self):
        ...
        os.rename(os.path.join(self.root, name), self.raw_dir)
    ...
    def process_filenames(self, filenames):
        ...
        for name in tq(filenames):
            cat = name.split(os.path.sep)[0]
            if cat not in categories_ids:
                continue

            data = read_txt_array(os.path.join(self.raw_dir, name))
            pos = data[:, :3]
            x = data[:, 3:6]
    ...
    def process(self):
        ...
        for i, split in enumerate(["train", "val", "test"]):
            path = os.path.join(self.raw_dir, "train_test_split", f"shuffled_{split}_file_list.json")
            with open(path, "r") as f:
                filenames = [
                    os.path.sep.join(name.split(os.path.sep)[1:]) + ".txt" for name in json.load(f)
